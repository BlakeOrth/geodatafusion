...
use geodatafusion_geoparquet_next::table::GeoParquetNextTable;

async fn setup_df(fixture: &str, use_flat: bool) -> DataFrame {
    ...
    let _ = ctx
        .register_table(
            "table",
            Arc::new(
                GeoParquetNextTable::new(inner, &ctx.state(), use_flat)
                    .await
                    .unwrap(),
            ),
        )
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let df_rtree = rt.block_on(setup_df(fixture, false));
    c.bench_with_input(
        BenchmarkId::new("intersects", format!("{}/rtree", fixture)),
        &df_rtree,
        |b, df| {
    ...
    let df_flat = rt.block_on(setup_df(fixture, true));
    c.bench_with_input(
        BenchmarkId::new("intersects", format!("{}/flat", fixture)),
        &df_flat,
        |b, df| {
            b.to_async(&rt).iter_batched_ref(
                || filter_df(df.clone()),
